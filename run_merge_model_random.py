...
from kw_02_core.model_merge import Model
...
if __name__ == '__main__':
    ...
    configs = json.load(open('config_merge.json', 'r', encoding='utf-8', errors='ignore'))  # 加载模拟配置json
    # 设置要进行模拟的站名
    # '北运河东关大桥','北运河榆林庄','凤港减河小屯（纪各庄）','港沟河后元化','凉水河马驹桥','凉水河许各庄','温榆河北关闸','玉带河玉带河入凉水河口'
    cs_names = ['北运河榆林庄']
    ...
    # 加载待融合模型
    model_path = r'.\kw_03_saved_models/'
    model_file_1 = '1_2.h5'  # input('请输入选用的待融合模型1文件名：')
    model_1 = model_path + model_file_1
    model_file_2 = '2_2.h5'  # input('请输入选用的待融合模型2文件名：')
    model_2 = model_path + model_file_2
    model = Model()
    mymodel = model.merge_model(model_1, model_2, configs)
    plot_model(mymodel, to_file='merge_model.png', show_shapes=True, show_layer_names=True, show_layer_activations=True)

    # choose_method = input('请输入要选择的模拟方式（1、all→cs  2、cs→cs  3、结束）：')
    # cs_cs(data_0, cs_names, configs)
    # 加载训练集数据
    # x, y = all_cs(data_0, cs_names, configs)
    data_cs = data_0[data_0['cs_name'].values == cs_names[0]][configs['data']['columns']]  # 依次提取各站点数据，并筛选出各波段数值作为模型输入
    data_cs = data_cs.reset_index(drop=True)
    ...
    for name in cs_names:
        ...
        split = configs['data']['train_test_split']
        i_split = int(len(data_0[data_0['cs_name'] == name]) * split)  # 划分比→训练集长度
        x_name = data_0[data_0['cs_name'] == name].get('date_month').values[(i_split+configs['data']['sequence_length']):]  # 划分数据集→测试集
        title = name + '_' + configs['data']['columns'][-1]     # 设置绘图标题和图名
        # 绘制反归一化后的预测值和真实值
